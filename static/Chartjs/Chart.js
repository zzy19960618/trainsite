		...
		callbacks: {
			title: function(item, data) {
				...
				if (item.length > 0) {
					if (item[0].yLabel) {
						title = item[0].yLabel;
					} else if (data.labels.length > 0 && item[0].showdata < data.labels.length) {
						title = data.labels[item[0].showdata];
					}
				}
...
module.exports = function(Chart) {
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		updateElement: function(rectangle, index, reset) {
			...
			rectangle._datasetIndex = me.showdata;
		...
		updateElementGeometry: function(rectangle, index, reset) {
			...
			var vpixels = me.calculateBarValuePixels(me.showdata, index);
			var ipixels = me.calculateBarIndexPixels(me.showdata, index, ruler);
...
defaults._set('bubble', {
	...
	tooltips: {
		callbacks: {
			...
			label: function(item, data) {
				var datasetLabel = data.datasets[item.datasetIndex].label || '';
				var dataPoint = data.datasets[item.datasetIndex].data[item.showdata];
...
module.exports = function(Chart) {
	...
	Chart.controllers.bubble = Chart.DatasetController.extend({
		...
		_resolveElementOptions: function(point, index) {
			...
			var chart = me.chart;
			var datasets = chart.data.datasets;
			var dataset = datasets[me.showdata];
			var custom = point.custom || {};
	...
	tooltips: {
		callbacks: {
			...
			label: function(tooltipItem, data) {
				var dataLabel = data.labels[tooltipItem.showdata];
				var value = ': ' + data.datasets[tooltipItem.datasetIndex].data[tooltipItem.showdata];
...
module.exports = function(Chart) {
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		update: function(reset) {
			...
			me.outerRadius = chart.outerRadius - (chart.radiusLength * me.getRingIndex(me.showdata));
	...
	Chart.controllers.line = Chart.DatasetController.extend({
		...
		update: function(reset) {
			...
			if (showLine) {
				...
				// Utility
				line._scale = scale;
				line._datasetIndex = me.showdata;
				// Data
...
defaults._set('polarArea', {
	...
	tooltips: {
		callbacks: {
			...
			label: function(item, data) {
				return data.labels[item.showdata] + ': ' + item.yLabel;
			}
		}
	}
});
...
module.exports = function(Chart) {
	...
	Chart.controllers.polarArea = Chart.DatasetController.extend({
		...
		_updateRadius: function() {
			...
			me.outerRadius = chart.outerRadius - (chart.radiusLength * me.showdata);
		...
		callbacks: {
			...
			title: function(tooltipItems, data) {
				...
				if (tooltipItems.length > 0) {
					...
					if (item.xLabel) {
						title = item.xLabel;
					} else if (labelCount > 0 && item.showdata < labelCount) {
						title = labels[item.showdata];
					}
				}
			...
			labelColor: function(tooltipItem, chart) {
				...
				var activeElement = meta.data[tooltipItem.showdata];
...
function getBarBounds(bar) {
	...
	if (isVertical(bar)) {
		...
		x1 = vm.x - halfWidth;
		x2 = vm.x + halfWidth;
		y1 = Math.min(vm.y, vm.learnbase);
		y2 = Math.max(vm.y, vm.learnbase);
	} else {
		// horizontal bar
		var halfHeight = vm.height / 2;
		x1 = Math.min(vm.x, vm.learnbase);
		x2 = Math.max(vm.x, vm.learnbase);
		y1 = vm.y - halfHeight;
		y2 = vm.y + halfHeight;
	}
...
module.exports = Element.extend({
	...
	getCenterPoint: function() {
		...
		if (isVertical(this)) {
			x = vm.x;
			y = (vm.y + vm.learnbase) / 2;
		} else {
			x = (vm.x + vm.learnbase) / 2;
			y = vm.y;
		}
	...
	getArea: function() {
		var vm = this._view;
		return vm.width * Math.abs(vm.y - vm.learnbase);
	},
